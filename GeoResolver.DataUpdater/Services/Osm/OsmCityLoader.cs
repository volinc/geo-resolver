...
out geom;";
		...
		foreach (var endpoint in _overpassEndpoints)
			try
			{
				...
				httpClient.Timeout =
					TimeSpan.FromMinutes(30); // Overpass queries for global data with multipolygon can take 20+ minutes

				var requestContent =
					new StringContent(overpassQuery, Encoding.UTF8, "application/x-www-form-urlencoded");
				...
				{
					_logger.LogWarning("Failed to query Overpass API at {Endpoint}: {StatusCode}", endpoint,
						response.StatusCode);
					continue;
				}

				_logger.LogInformation(
					"Response received, reading content (this may take several minutes for large datasets)...");
				...
				_logger.LogInformation(
					"Successfully loaded cities from OpenStreetMap via {Endpoint} in {ElapsedMilliseconds}ms ({ElapsedMinutes:F2} minutes)",
		...
		throw new InvalidOperationException(
			"Failed to load cities from OpenStreetMap. Please check Overpass API availability.", lastException);
	}
    ...
    private string ConvertOverpassToGeoJson(string overpassJson)
	{
		try
		{
			...
			foreach (var element in elements.EnumerateArray())
			{
				...
				else if (typeStr == "way") ways.Add(element);
			}
			...
			foreach (var relation in relations)
			{
				...
				if (feature.HasValue) features.Add(feature.Value);
			}
	...
	private JsonElement? ConvertRelationToFeature(JsonElement relation,
		Dictionary<(string type, long id), JsonElement> elementsById)
	{
		try
		{
			...
			{
				_logger.LogDebug("Relation {RelationId} has no valid outer ways",
			...
			foreach (var outerWay in outerWays)
			{
				...
				if (multiPolygonCoordinates.Count == 0) polygon.AddRange(innerWays);
	...
	private List<double[]>? ExtractWayCoordinates(JsonElement way)
	{
		try
		{
			if (!way.TryGetProperty("geometry", out var geometry) ||
	...
	private JsonElement? ConvertWayToFeature(JsonElement way)
	{
		...
		{
			if (!way.TryGetProperty("geometry", out var geometry) ||
			    !geometry.ValueKind.Equals(JsonValueKind.Array)) return null;
