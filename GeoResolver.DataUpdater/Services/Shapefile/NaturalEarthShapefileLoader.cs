...
public class NaturalEarthShapefileLoader
{
    ...
    public async Task LoadTimezonesAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var url in downloadUrls)
        {
            try
            {
                ...
                overallStopwatch.Stop();
                _logger.LogInformation("Successfully loaded timezones from {Url} in {ElapsedMilliseconds}ms ({ElapsedMinutes:F2} minutes)", 
                    url, overallStopwatch.ElapsedMilliseconds, overallStopwatch.Elapsed.TotalMinutes);
                return;
            }
            catch (HttpRequestException ex)
        ...
        _logger.LogInformation("1. Download timezones.geojson.zip from: https://github.com/evansiroky/timezone-boundary-builder/releases");
        ...
        throw new InvalidOperationException("Failed to download timezones GeoJSON. Please download manually from https://github.com/evansiroky/timezone-boundary-builder/releases", lastException);
    }
    ...
    private async Task ProcessTimezonesGeoJsonZipAsync(Stream zipStream, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (geoJsonFile == null)
            {
                _logger.LogError("GeoJSON file not found in ZIP archive. Files found: {FileCount}", allFilesInArchive.Length);
            ...
            overallStopwatch.Stop();
            _logger.LogInformation("Timezones GeoJSON processing completed in {ElapsedMilliseconds}ms ({ElapsedMinutes:F2} minutes)", 
                overallStopwatch.ElapsedMilliseconds, overallStopwatch.Elapsed.TotalMinutes);
        }
        finally
        {
